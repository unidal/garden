...
public class UmlServlet extends HttpServlet {
	...
	private List<String> m_docs;

	private void createUml(HttpServletRequest req, UmlViewModel model)
			throws IOException {
		...
		if (!umlFile.endsWith(".uml")) {
			...
			model.setMessage(String.format(
					"Target UML file(%s) must be ending with '.uml'!", umlFile));
			...
			model.setError(true);
			model.setMessage(String.format(
					"UML File(%s) is already existed! Please use another one.",
					umlFile));
		}
	}
	...
	public void init(ServletConfig config) throws ServletException {
		String docs = config.getInitParameter("docs");

		if (docs == null) {
			m_docs = Arrays.asList("doc");
		} else {
			m_docs = Splitters.by(',').noEmptyItem().trim().split(docs);
		}
	...
	private void updateUml(HttpServletRequest req, UmlViewModel model)
			throws IOException {
		String uml = req.getParameter("uml");
		String umlFile = req.getParameter("file");
		StringBuilder message = new StringBuilder();
